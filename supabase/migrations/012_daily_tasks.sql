-- Daily mission tracking: one count per actual game entry, daily rewards are idempotent.

create table if not exists public.gamezcoin_daily_game_entries (
  user_id uuid not null references auth.users(id) on delete cascade,
  task_date date not null,
  entry_id text not null check (char_length(entry_id) between 8 and 160),
  game_id text not null references public.gamezcoin_games(id),
  created_at timestamptz not null default now(),
  primary key (user_id, task_date, entry_id)
);

create index if not exists gamezcoin_daily_game_entries_user_date_idx
  on public.gamezcoin_daily_game_entries(user_id, task_date, created_at desc);

alter table public.gamezcoin_daily_game_entries enable row level security;
revoke all on table public.gamezcoin_daily_game_entries from anon, authenticated;
...
begin
  if p_task = 'play10' then
    ...
    where user_id = p_user_id and task_date = v_date;
...
begin
  ...
  insert into public.gamezcoin_daily_game_entries(user_id, task_date, entry_id, game_id)
  values(p_user_id, v_date, p_entry_id, p_game_id)
