-- Make finish_game safe to retry after transient browser / Edge Function failures.
-- A rewarded session returns the stored result instead of creating a second reward.
...
begin
  ...
  if not found or v_session.user_id<>p_user_id then
    return jsonb_build_object('rejected',true,'reason','SESSION_NOT_FOUND');
  end if;

  if v_session.status='rewarded' then
    return jsonb_build_object(
      'rejected',false,
      'score',coalesce(v_session.client_score,p_score),
      'game_coin',coalesce(v_session.awarded_coin,0),
      'replayed',true,
      'referral_inviter_coin',0,
      'referral_invitee_coin',0
    );
  end if;

  if v_session.status='rejected' then
    return jsonb_build_object('rejected',true,'reason',coalesce(v_session.reject_reason,'SESSION_ALREADY_FINISHED'));
  end if;

  if v_session.status<>'active' then
    return jsonb_build_object('rejected',true,'reason','SESSION_ALREADY_FINISHED');
  end if;
  ...
  update public.gamezcoin_game_sessions
  set status='rewarded',client_score=p_score,awarded_coin=v_coin,finished_at=now()
  where id=p_session_id;
  ...
  if v_coin>0 then
    ...
    if found then
      ...
      set balance=balance+v_coin,lifetime_earned=lifetime_earned+v_coin,updated_at=now()
      where user_id=p_user_id;
    end if;
  end if;

  select count(*) into v_game_reward_count
  from public.gamezcoin_wallet_ledger
  where user_id=p_user_id and entry_type='game_reward';
  ...
  if v_game_reward_count=1 then
    ...
    if found then
      ...
      v_inviter_bonus:=coalesce(v_inviter_bonus,0);
      v_invitee_bonus:=coalesce(v_invitee_bonus,0);
  ...
  return jsonb_build_object(
    ...
    'score',p_score,
    'game_coin',v_coin,
    'points_per_coin',case when v_game.id='block-blast' then v_points_per_coin else null end,
    'replayed',false,
    'referral_inviter_coin',v_inviter_bonus,
    'referral_invitee_coin',v_invitee_bonus
  );
end;
$function$;
